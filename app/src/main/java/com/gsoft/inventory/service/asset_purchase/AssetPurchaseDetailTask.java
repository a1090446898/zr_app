 ...
 * @author 10904
 */
public class AssetPurchaseDetailTask implements Callable<List<AssetPurchaseDetailItem>> {

    private String docId;

    public AssetPurchaseDetailTask(String docId){
        this.docId = docId;
    }

    @Override
    public List<AssetPurchaseDetailItem> call() throws Exception {
        String urlParam = String.format(SysDefine.getCgDetailList(), docId);
        ...
        List<AssetPurchaseDetailItem> dataList = new ArrayList<>();
        for(String dataLine : results){
            ...
            if(rowStrArray.length > 0){
                AssetPurchaseDetailItem item = new AssetPurchaseDetailItem();
