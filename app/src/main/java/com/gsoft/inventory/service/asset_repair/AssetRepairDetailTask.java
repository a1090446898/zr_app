 ...
 * @author 10904
 */
public class AssetRepairDetailTask implements Callable<List<AssetRepairDetailItem>> {

    private String docId;

    public AssetRepairDetailTask(String docId){
        this.docId = docId;
    }

    @Override
    public List<AssetRepairDetailItem> call() throws Exception {
        String urlParam = String.format(SysDefine.getBxDetailList(), docId);
        ...
        List<AssetRepairDetailItem> dataList = new ArrayList<>();
        for(String dataLine : results){
            ...
            if(rowStrArray.length > 0){
                AssetRepairDetailItem item = new AssetRepairDetailItem();
