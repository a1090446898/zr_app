 ...
 * @author 10904
 */
public class AssetReceiveDetailTask implements Callable<List<AssetReceiveDetailItem>> {

    private String docId;

    public AssetReceiveDetailTask(String docId){
        this.docId = docId;
    }

    @Override
    public List<AssetReceiveDetailItem> call() throws Exception {
        String urlParam = String.format(SysDefine.getLyDetailList(), docId);
        ...
        List<AssetReceiveDetailItem> dataList = new ArrayList<>();
        for(String dataLine : results){
            ...
            if(rowStrArray.length > 0){
                ...
                AssetReceiveDetailItem item = new AssetReceiveDetailItem();
